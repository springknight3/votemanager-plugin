...
```text
/give @s stick[custom_data={vote:{redeemable:{},on_redeem:"shards give <user> 1000"}}]
```

When redeemed, the plugin executes `vote.on_redeem` as the console command and
replaces `<user>` with the player name. One item is removed only when the
command succeeds; otherwise `redeemable_item_error` is sent.
